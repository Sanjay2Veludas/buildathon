...
def test_mcu_connection():
    """Check MCU serial connection with plain-text command protocol."""
    ...
    try:
        ...
        for port in ports:
            try:
                ...
                ser.close()
                if got_pong:
                    print("  ✓ MCU command channel responsive (pong)")
                    return True
                print("  ✗ MCU connected but no pong response")
            except (serial.SerialException, FileNotFoundError):
                pass

        print("  ✗ No responsive MCU found on common ports")
        return False
    except ImportError:
